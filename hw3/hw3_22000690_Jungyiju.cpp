...
int find_MAX(struct c a, struct c b){
...
int main() {
    ...
    for(int i = 0; i < 1; i++){  // Execute All the jobs
...
void act_print(int (*c)[11], int (*act)[11], int i, int j){
    if (c[i][j] > 0){
        int k = act[i][j];
        cout << k;
        act_print(c, act, i, k);
